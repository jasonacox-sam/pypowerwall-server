...
Use this grid to pick the right connection mode for your setup:

| Mode | What You Need | Environment Variables | Best For | Data |
|------|---------------|----------------------|----------|------|
| **TEDAPI** | Gateway password (Tesla app → Gateway settings) | `PW_HOST` + `PW_GW_PWD` | Local network access to the gateway — fastest and most complete | Full |
| **TEDAPI v1r** | Gateway password + RSA-4096 private key | `PW_HOST` + `PW_GW_PWD` + `PW_RSA_KEY_PATH` (+ `PW_WIFI_HOST` optional) | Wired LAN access to the gateway with the encrypted v1r channel | Full |
| **Basic LAN** (PW3) | Customer password (last 5 chars of gateway password) + Powerwall wired LAN IP (vendor subnet, e.g. `10.42.1.x`) | `PW_HOST` + `PW_PASSWORD` | Powerwall 3 over Ethernet without the gateway password or RSA key | Core metrics |
| **Cloud** | Tesla account login (one-time `--setup` auth flow) | `PW_EMAIL` + `PW_AUTH_PATH` | Remote access from anywhere; no local network access required | Basic metrics + control |
| **FleetAPI** | Registered Tesla developer app + FleetAPI auth tokens | `PW_GATEWAYS` / `gateways.yaml` with `email` + `authpath` + `fleetapi: true` | Official Tesla API access with your own registered application | Basic metrics + control |
| **Cloud Control** (hybrid) | Local TEDAPI creds **and** Tesla account login | `PW_HOST` + `PW_GW_PWD` + `PW_EMAIL` + `PW_AUTH_PATH` | Local reads (full data) combined with cloud control (reserve, mode) | Full + control |

Notes:
- `PW_HOST` points at the gateway (`192.168.91.1` on its WiFi) or, for Basic LAN, the Powerwall's wired IP.
- Cloud and FleetAPI modes use token files (`.pypowerwall.auth`, `.pypowerwall.site`) generated by the one-time setup below.
- FleetAPI requires per-gateway `fleetapi: true` in `PW_GATEWAYS` JSON or the [gateways.yaml](#configuration-file-gatewaysyaml) config file — it cannot be enabled with legacy single-gateway environment variables alone.

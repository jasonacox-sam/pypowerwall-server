...
- **Firmware version logging** — gateway firmware is logged at first successful poll and on every change (`Gateway <id> firmware changed: X -> Y`), so `docker logs` answers "when did my firmware update?" (Powerwall-Dashboard #854)
